...
原始调研文件：`docs/raw/01~05-*.md`、`docs/raw/14-transform-operator-library.md`，以及 `docs/raw/pipeline-builder-operators/` 中迁入的算子总结、HTML 总览和 final bundle。
...
本节补充吸收 `docs/raw/pipeline-builder-operators/` 中迁入的算子调研总结、最终清单和 HTML 总览页。该调研已经把 Pipeline Builder 算子拆成两条主线：转换算子（transform）负责数据集级转换，表达式函数（expression function）负责字段和值级表达。【事实，来自 `docs/raw/pipeline-builder-operators/pipeline-builder-operators-research-summary.md` 与 `deliverables/pages/pipeline-builder-operators-overview.html`】
...
迁入材料确认，当前调研产物已经形成两套 final bundle：【事实，来自 `docs/raw/pipeline-builder-operators/` final bundle 说明】

| 主线 | 数量 | 核心定位 | 正式入口 |
|---|---:|---|---|
| transform | 89 条 | 数据集级转换，描述 Pipeline Builder 中的处理步骤 | `docs/raw/pipeline-builder-operators/artifacts/transform-final/README.md` |
| expression | 335 条 | 字段和值级表达，常作为 transform 参数或列级逻辑出现 | `docs/raw/pipeline-builder-operators/artifacts/pb-expression-final/README.md` |

transform 的代表性场景包括 Aggregate、Pivot、Window、Mapping join、Geometry intersection join、Parse Excel、Parse KML、First union by name、Key by 和 Rollup。expression 的重点类别包括 Numeric、Boolean、Data preparation、String、Datetime、Cast、Array、Struct、JSON 和 Map。【事实，来自 `deliverables/pages/pipeline-builder-operators-overview.html`】
...
证据边界也需要进入全局调研：两套 final bundle 是正式清单入口；自动解析、stage、verify 和一次性渲染产物只适合作为追溯材料，不应直接作为正式结论引用。Batch、Faster、Streaming 等字段是当前可见的支持环境标签，不等价于 Palantir 内部执行引擎实现。【事实 + 推断，来自 `docs/raw/pipeline-builder-operators/README.md` 与概要设计文档】
...
迁入概要设计把平台落地架构概括为“算子注册中心 + 类型化中间表示 + 执行适配器”。这与本仓库原有的 Spec / Executor 分层判断一致：注册中心负责能力事实入口，流水线中间表示负责画布、校验、编译和审计之间的稳定边界，执行适配器负责把同一套语义计划映射到 Spark、Flink、文件解析、地理空间、媒体处理和自定义函数等运行环境。【推断，来自 `docs/raw/pipeline-builder-operators/pipeline-builder-operator-platform-architecture-design.md`】
...
- `docs/raw/pipeline-builder-operators/pipeline-builder-operators-research-summary.md`
- `deliverables/pages/pipeline-builder-operators-overview.html`
- `docs/raw/pipeline-builder-operators/pipeline-builder-operator-platform-architecture-design.md`
- `docs/raw/pipeline-builder-operators/artifacts/transform-final/README.md`
- `docs/raw/pipeline-builder-operators/artifacts/pb-expression-final/README.md`
