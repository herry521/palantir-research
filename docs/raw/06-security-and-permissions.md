...
1. **Marking 传播是最容易踩的坑**：上游数据集加了 Marking，所有下游都会继承，需提前用 Lineage 模拟影响范围
2. **行级安全优先用 Ontology 对象策略**：比 Restricted View 更灵活，且与应用层无缝集成
3. **Cipher 解决了"数据必须加密存储"的合规需求**：但增加了 Pipeline 编写复杂度
4. **SDS（2024）填补了敏感数据发现盲区**：对于大型组织，自动扫描比手工分类更可靠
5. **Foundry 安全模型强绑定平台**：无法用标准 RBAC 框架（如 OPA）替代，是平台锁定成本的组成部分
