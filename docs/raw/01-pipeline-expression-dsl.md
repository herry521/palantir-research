...
1. **装饰器即声明**：`@transform_df` 通过装饰器静态声明输入输出，Foundry 运行时基于此构建 DAG，函数本身只关注业务逻辑 [事实]
2. **`pipeline.py` 是 DAG 注册入口**：所有 Transform 必须通过 Pipeline 对象注册才能被 Foundry 调度系统感知 [事实]
3. **路径即依赖**：Foundry DAG 以数据集路径为节点，两个 Transform 共享同一路径即建立依赖关系，无需显式声明 [事实]
4. **平台级依赖管理统一血缘**：Foundry Automation dependencies 机制跨 Repository 统一管理依赖，是平台实现全局血缘图的基础（注："FDS/Foundry Dependency Services"为非官方术语，避免使用） [推断]
5. **与 dbt 的本质区别**：Foundry Transforms 是 Spark 原生执行（支持任意 PySpark 代码），dbt 仅处理 SQL；Foundry 的 Ontology 集成是 dbt 没有的核心差异 [事实]
