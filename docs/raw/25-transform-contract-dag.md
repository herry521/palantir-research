...
| 标签 | 含义 | 使用方式 |
|---|---|---|
| 【实时】 | 本轮已由 Palantir 官方公开文档或仓库既有资料直接确认 | 可作为事实基线 |
| 【推断】 | 多个【实时】事实组合出的工程判断，但官方未以同一句话公开说明 | 可进入架构建议，但需保留边界 |
| 【猜测】 | 官方公开资料未披露，或证据不足 | 只作为验证问题，不作为设计前提 |

注意：沿用项目既有“实时”标签名，含义是“当前已验证事实”，不是“实时流处理能力”。

---

## 核心结论

1. Foundry Transform Contract 的核心不是普通函数签名，而是“输入数据集、输出数据集、计算函数、运行配置、Pipeline 注册入口”的组合元数据。【实时】Java 官方文档明确称 `Transform` 描述如何计算一个 dataset，包含 input/output datasets、compute function 和额外配置；这些信息被指定在 `Transform` 对象中并注册到 `Pipeline`。
2. Python transform 通过 `Input()` / `Output()` 与装饰器声明数据依赖，`Pipeline` 作为 Transform 对象注册表，默认 `pipeline.py` 使用 `discover_transforms(datasets)` 自动发现 `datasets` 包中的 Transform。【实时】
3. Java transform 通过 `@Input` / `@Output` / `@Compute` 注解表达 contract，并通过 `PipelineDefiner`、`Pipeline.autoBindFromPackage()` 或 `Pipeline.register()` 注册到 Pipeline。【实时】
4. SQL transforms 是 Foundry 高码路径之一，使用 Spark SQL 表达转换，但官方明确 SQL transforms 不支持 incremental transforms。【实时】
5. Foundry 调度围绕 Dataset 与 Data Lineage 图组织：schedule 可以按时间、数据更新、逻辑更新触发，并可构建单个 dataset、dataset 及其依赖、依赖某 dataset 的全部下游，或两个 dataset 之间的连接集合。【实时】
...
- Python 默认项目结构包含 `pipeline.py`；官方称该文件定义项目的 `Pipeline`，即与数据转换关联的 `Transform` 对象注册表。【实时】
- Python 默认 `pipeline.py` 示例为 `my_pipeline = Pipeline()` 与 `my_pipeline.discover_transforms(datasets)`；官方说明自动注册会发现项目 `datasets` 包中的所有 Transform 对象。【实时】
- Python `setup.py` 默认导出 `transforms.pipelines` entry point，运行时需要通过该入口找到项目的 `Pipeline` 对象。【实时】
- Java 每个 transforms Java subproject 暴露一个 `Pipeline` 对象；官方称该对象用于“注册 Foundry 中 dataset 的构建说明”并在 build 时定位负责构建目标 dataset 的 `Transform`。【实时】
- Java high-level / low-level Transform 的差异影响 contract 表达：high-level 返回 `Dataset<Row>`，low-level 显式写 output，low-level 支持多输出与文件访问。【实时】
- Foundry 资源有 RID，第三方 BI 文档确认 Dataset 可用 RID 或 Location/filepath 指定；API 文档也把 dataset RID 作为 Dataset API path parameter。【实时】
...
Python 中，开发者使用 `Input('/path')`、`Output('/path')` 和 `@transform` / `@transform_df` / `@transform.using` / `@transform.spark.using` 声明输入输出。【实时】官方示例展示了单输入单输出、多输入多输出，以及 PySpark `transform_df` 直接注入 DataFrame 的形式。

Java 中，开发者使用 `@Input("/path")`、`@Output("/path")`、`@Compute` 注解声明输入、输出与计算函数。【实时】官方文档要求 compute function 是 public、non-static，并标注 `@Compute`，否则不会正确注册。

SQL 中，转换逻辑以 Spark SQL 表达；公开概览只确认 SQL transforms 属于 Code Repositories batch pipeline 路径，未在 overview 中展开 Python/Java 同级的注解机制。【实时】

### 2. Registration 阶段：把 transform 暴露给运行时

Python 的 `Pipeline` 是 Transform 对象注册表，自动注册通过 `discover_transforms()` 递归发现模块或包内 Transform；手动注册通过 `add_transforms()` 加入并检查是否有两个 Transform 声明同一 output dataset。【实时】

Java 的自动注册通过 `Pipeline.autoBindFromPackage()` 发现包中带所需 `@Input` / `@Output` 注解的 Transform；手动注册通过 `Pipeline.register()`。【实时】
...
Transform Contract 中的 `Input` / `Output` 引用通常在代码示例中表现为 dataset path/location。【实时】Foundry 平台同时为 resource 提供 RID，并且 Dataset API 使用 dataset RID 获取 dataset。【实时】
...
Data Lineage 官方文档确认用户可展开 dataset 的 ancestors/descendants，查看 schema、last build time、生成数据的 code，并能跳转到 repository 中的原始代码。【实时】这说明 Foundry 至少保存了 dataset 节点、父子关系、构建状态与代码 provenance 的可查询元数据。

### 5. Scheduling 阶段：从 graph 选择构建范围

官方 Scheduling 文档确认 scheduled builds 可由时间、数据更新、逻辑更新触发，也可构建单个 dataset、单个 dataset 及其 dependencies、依赖某 dataset 的全部下游、连接两个 dataset 的全部 datasets 等范围。【实时】

Create schedule 文档确认 schedule editor 位于 Data Lineage，并且 target datasets、excluded datasets 会影响 graph traversal；dataset 更新可作为事件触发条件。【实时】

因此 Foundry 的调度心智更接近“Dataset graph 上的构建范围选择”，而不是 Airflow 式“预先手写 task DAG 后按 task 执行”。【推断】

### 6. Incremental 与 staleness

Python incremental 文档确认 `@incremental` 可叠加到 transforms，并使用 `snapshot_inputs`、retention 相关配置等影响增量可行性；输入发生非增量兼容变更时会触发 snapshot。【实时】

Java incremental 文档确认 API 可读取 input modification type，并据此选择 `ReadRange` 与 `WriteMode`，例如 append-only 可读 unprocessed，updated/new view 可读 entire view 或写 snapshot。【实时】
...
以上差异中，Python/Java 的 Input/Output contract 和 Pipeline 注册均有官方直接支撑。【实时】SQL 的 contract 细节公开资料不足，只能确认其作为 Code Repositories SQL transforms 路径以及不支持 incremental。【实时】
...
|---|---|---|---|---|
| 主要抽象 | Dataset + Transform Contract + Lineage graph【推断】 | Task/operator DAG【实时】 | SQL model + `ref()` 依赖【实时】 | Asset/op graph【实时】 |
| 依赖声明 | Input/Output dataset refs【实时】 | Python DAG 中显式 task dependency【实时】 | `ref()` / source / YAML【实时】 | asset keys / deps / op graph【实时】 |
| 调度范围 | 可按 dataset、dependencies、downstream、between datasets 选择【实时】 | 通常按 DAG run/task run 组织【推断】 | 通常按 selector/model graph 组织【推断】 | 可按 assets/jobs/schedules/sensors 组织【推断】 |
| 血缘 | Data Lineage app 展示 ancestors/descendants、build、code provenance【实时】 | 依赖外部 lineage 集成或 operator 元数据【推断】 | model graph 内置，跨系统 lineage 需集成【推断】 | asset lineage 是核心概念【实时】 |
| 计算逻辑 | Python/Java/SQL 等 Foundry 托管 transforms【实时】 | 任意 operator，平台不天然理解业务数据输出【推断】 | SQL 为主【实时】 | Python 定义资产/ops，执行器可插拔【实时】 |
| 平台绑定 | 强绑定 Foundry Dataset/RID/权限/lineage【推断】 | 开源编排器【实时】 | 数仓/适配器生态【实时】 | 开源编排与资产平台【实时】 |
...
4. 在注册阶段检查同一 branch/scope 下的 output 唯一性；Python 官方 `add_transforms()` 会检查两个 Transform 是否声明相同 output dataset，可作为设计参照。【实时】
