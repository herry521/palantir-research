...
## 八、DataFusion 与数据湖格式的适配性

### 8.1 四种主流格式集成对比

| 格式 | Rust 实现 | DataFusion 读 | DataFusion 写 | 成熟度 |
|---|---|---|---|---|
| **Delta Lake** | `delta-rs` ✅ | ✅ 完整 | ✅ 完整（DELETE/UPDATE/MERGE/OPTIMIZE） | 生产级 |
| **Iceberg** | `iceberg-rust` ✅ | ✅ 完整 | ⚠️ INSERT 支持，UPDATE/MERGE 开发中 | 快速演进中（v0.9.0，2026-03） |
| **Paimon** | `paimon-rs` ⚠️ | ✅ 只读 | ❌ 不支持 | 早期 |
| **Hudi** | 无原生 Rust 实现 | ❌ 无 | ❌ 无 | 不支持 |

### 8.2 Delta Lake 是最适配格式

**结论：Delta Lake（通过 `delta-rs`）是目前与 DataFusion 集成最成熟的数据湖格式。**

根因有三：

**① 同源技术栈，全链路零序列化**

`delta-rs` 和 DataFusion 均为 Rust + Apache Arrow 原生实现。Delta Lake 的数据文件就是标准 Parquet，事务日志是 JSON，无 JVM 依赖。数据从 Parquet 读入即为 Arrow RecordBatch，不经任何转换层：

```
DataFusion（查询）─── Arrow RecordBatch ───► delta-rs（读写）
                            ↑
                     标准 Parquet 文件
                   （无 LSM merge，无额外 overhead）
```

对比：Paimon MOR 需要读时 merge 多层 LSM 文件；Hudi 深度绑定 JVM 无 Rust 路径。

**② Parquet 下推优化完全生效**

Delta Lake 不改变 Parquet 文件本身结构，DataFusion 的四层下推剪枝（文件级 / RowGroup 级 / Page 级 / 行级延迟物化）对 Delta 表完全适用。Paimon Primary Key 表的 LSM 结构会导致列统计信息跨层重叠，剪枝效果打折。

**③ `delta-rs` 写支持完整且经生产验证**

已支持 DELETE、UPDATE、MERGE INTO、OPTIMIZE（Z-Order）、Schema Evolution、Time Travel、并发写（OCC）。Scribd、DoorDash 等已在生产环境使用 `delta-rs` + DataFusion 做高吞吐写入。

### 8.3 各格式适配 Paimon 的特殊分析

Paimon 读取效率取决于表类型：

| Paimon 表类型 | DataFusion 读效率 | 原因 |
|---|---|---|
| Append-Only（无主键） | ≈ 纯 Parquet，高效 | 文件布局简单，下推优化生效 |
| Primary Key + MOR（默认） | 明显劣化 | LSM 多层 merge + 读并发受 bucket 数限制 |
| Primary Key + MOW/Deletion Vector | 接近高效 | DV 文件过滤开销小，无全量 merge |
| Primary Key + COW | 接近高效（但写极慢） | 写时已完成 compaction，读无 merge |

**写支持现状**：`paimon-datafusion` 当前只读，无写路径，写 Paimon 仍需 Spark / Flink。

### 8.4 选型建议

| 需求场景 | 推荐格式 |
|---|---|
| DataFusion 为主引擎，需完整读写 | **Delta Lake**（delta-rs） |
| 多引擎共读（Spark/Trino/Snowflake） | **Iceberg**（接受 DML 不完整） |
| CDC / 高频 Upsert / 实时写入 | Paimon 或 Hudi（需配合 Flink，放弃 DataFusion 写路径） |
| 纯 Append 分析型 | 三者均可，Delta Lake 最省事 |

> **证据：** `delta-rs` GitHub 及官方文档；`iceberg-rust` v0.9.0 release notes（2026-03）；`paimon-datafusion` crate 文档（只读限制明确标注）；Hudi 无 Rust 实现为社区公知事实。
