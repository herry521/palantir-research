...
13. 自建 Dataset 的血缘建设顺序应是：先建 Dataset 版本底座，再注册 Pipeline/Sync 的声明式依赖，最后把每次运行绑定到具体输入输出版本；否则只能得到任务 DAG，得不到可回放、可审计、可解释的版本血缘。【推断】
...
自建 Dataset 血缘不要从“画任务 DAG”开始，而要先建设 Dataset Control Plane。血缘的主语不是 job，而是 `Dataset + branch + transaction/view + producer run`。Pipeline、Sync、Schedule、Ontology 和应用都应围绕这个版本坐标系登记关系。【推断】
...
External Source
  -> SyncRun
  -> Raw Dataset / branch / transaction
  -> BuildRun / Transform code version
  -> Curated Dataset / branch / transaction
  -> Object Type / API / App / Report
```

这套模型需要同时回答三类问题：【推断】

- 结构问题：谁依赖谁，改上游会影响哪些 Dataset、对象或应用；
- 版本问题：这个输出版本读了哪些输入 branch、transaction 或 transaction range；
- 运行问题：是哪次 SyncRun/BuildRun、哪个代码/配置版本生产了这个结果。

### 9.2 最小元数据模型

建议把血缘拆成以下核心实体，先保证 Dataset 级血缘可信，再逐步扩展列级血缘：【推断】

| 实体 | 关键字段 | 说明 |
|---|---|---|
| `dataset` | `dataset_id`, `path`, `owner`, `storage_type` | 数据资产稳定身份 |
| `dataset_branch` | `dataset_id`, `branch_name`, `head_transaction_id`, `parent_branch` | 数据分支指针，不等同于 Git merge |
| `dataset_transaction` | `transaction_id`, `type`, `state`, `schema_version`, `producer_run_id`, `file_manifest` | 每次写入的不可变版本事实 |
| `pipeline_def` / `transform_def` | `logic_id`, `code_version`, `config_version`, `input_refs`, `output_refs` | 静态声明式 I/O contract |
| `sync_def` | `source_ref`, `target_dataset_id`, `config_version`, `cursor_policy` | 外部源进入平台的入口 |
| `run` | `run_id`, `run_type`, `status`, `started_at`, `finished_at` | SyncRun / BuildRun 的统一运行记录 |
| `run_input` | `run_id`, `dataset_id`, `requested_branch`, `resolved_branch`, `transaction_range` | 运行时实际读取版本 |
| `run_output` | `run_id`, `dataset_id`, `branch_name`, `transaction_id` | 运行时实际产出版本 |
| `lineage_edge` | `from_ref`, `to_ref`, `edge_type`, `branch_scope`, `run_id` | 面向查询和 UI 的血缘索引 |

其中 `run_input.resolved_branch` 非常关键。用户在 feature branch 运行 Pipeline 时，输入可能因为 fallback 读到了 main branch；如果只记录 requested branch，后续排障和影响分析会误导用户。【推断】

### 9.3 分层建设路线

建议按以下顺序落地，避免一开始承诺过重的列级血缘和跨系统全图能力：【推断】

| 阶段 | 建设重点 | 产出能力 |
|---|---|---|
| P0 | Dataset transaction、branch、schema version、SyncRun/BuildRun 输入输出版本 | 版本追溯、审计、重放、基础上下游查询 |
| P1 | Pipeline/Sync 声明式 I/O、branch/fallback resolution、staleness | 影响分析、分支视图、调度范围选择 |
| P2 | SQL/低码表达式列级血缘、schema change impact、标签/权限传播 | 字段影响分析、治理自动传播 |
| P3 | OpenLineage 导入导出、跨系统 source/app/report graph、ontology/writeback | 跨平台资产图和业务语义闭环 |

### 9.4 Pipeline、Sync 和分支的落地规则

1. Pipeline 必须声明输入输出 Dataset，运行时必须记录实际读取的 branch、transaction/view/range 和输出 transaction。不要依赖运行日志反推血缘。【推断】

2. Sync 必须是一等血缘生产者，至少记录外部源、sync config version、cursor/watermark、transaction type 和 output transaction。外部源变化无法完全靠内部 DAG 感知，因此 Sync 的 schedule、manual run、connector state 都应进入审计链路。【推断】

3. Dataset branch 不应承诺 Git 式数据 merge。把分支数据带回主线时，应表现为 `promote`、`copy` 或 `rebuild-to-target-branch`，并生成新的 dataset transaction。【推断】

4. 版本血缘查询应优先支持 `ancestors(version)`、`descendants(version)`、`compare_versions(a,b)`、`why_stale(dataset)`、`branch_view(branch)` 五类 API/UI 场景。【推断】

5. 列级血缘应按可靠性分层：SQL/低码表达式可自动解析；Spark/Python 等任意代码先要求显式 annotation 或只做 best-effort，不应把“完全自动列级血缘”作为早期承诺。【推断】
